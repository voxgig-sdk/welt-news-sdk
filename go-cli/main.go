// Command WeltNews-cli is a CLI + REPL for the WeltNews SDK,
...
const prompt = "welt-news"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "article"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      WELT_NEWS_APIKEY (api key), WELT_NEWS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: WELT_NEWS_APIKEY carries the API key and
	// WELT_NEWS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WELT_NEWS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WELT_NEWS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWeltNewsSDK(opts)
