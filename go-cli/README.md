...
boru-driven command-line client **and** interactive REPL for the WeltNews
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/welt-news-cli)
make build
...
./welt-news-cli list article

# 5. Override the API base URL for a single call
WELT_NEWS_BASE=https://api.example.com ./welt-news-cli list article

# 6. No arguments -> interactive REPL
./welt-news-cli
welt-news> list article
welt-news> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/welt-news-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/welt-news-cli list article
   ```
...
Run with no arguments to open a REPL (prompt `welt-news>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/welt-news-cli`. |
...
(`./welt-news-cli <expr>`) and interactively (the REPL), and expressions compose the
