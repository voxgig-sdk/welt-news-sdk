...
import (
	...
	omni "github.com/voxgig-sdk/welt-news-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WeltNewsError); is {
				out := map[string]any{
					"name":    "WeltNewsError",
...
func omniCtx(arg any, client *sdk.WeltNewsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WeltNewsSDK); is {
			client = live
