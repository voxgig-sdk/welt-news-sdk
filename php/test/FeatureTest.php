...
class FtCtrl extends WeltNewsControl
{
...
class FtClient
{
    ...
    private WeltNewsUtility $utility;

    public function __construct(WeltNewsUtility $utility, array $options)
    {
    ...
    public function get_utility(): WeltNewsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WeltNewsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WeltNewsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WeltNewsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WeltNewsConfig::make_feature($name);
    ...
    private static function buildUrl(WeltNewsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WeltNewsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WeltNewsResponse($fetched);
        }
    ...
    private function fail(WeltNewsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WeltNewsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
