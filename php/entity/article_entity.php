...
class ArticleEntity
{
    ...
    {
        if ($args) {
            $this->_data = WeltNewsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = WeltNewsHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws WeltNewsError on failure (item-5 convention).
     */
