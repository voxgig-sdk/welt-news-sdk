...
class WeltNewsRetryFeature extends WeltNewsBaseFeature
{
    ...
    public function init(WeltNewsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WeltNewsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WeltNewsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WeltNewsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
