...
class WeltNewsMakeError
{
    public static function call(?WeltNewsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WeltNewsContext([], null);
        }
        $op = $ctx->op ?? new WeltNewsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WeltNewsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WeltNewsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new WeltNewsError('', $msg, $ctx);
        ...
        if ($err instanceof WeltNewsError) {
