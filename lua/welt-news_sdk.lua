...
local WeltNewsSDK = {}
WeltNewsSDK.__index = WeltNewsSDK
...
WeltNewsSDK._make_feature = _make_feature


function WeltNewsSDK.new(options)
  local self = setmetatable({}, WeltNewsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function WeltNewsSDK:options_map()
...
function WeltNewsSDK:get_utility()
  return Utility.copy(self._utility)
end


function WeltNewsSDK:get_root_ctx()
  return self._rootctx
end


function WeltNewsSDK:prepare(fetchargs)
...
function WeltNewsSDK:direct(fetchargs)
...
-- by passing it.
function WeltNewsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function WeltNewsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "WeltNewsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function WeltNewsSDK:Article(data)
...
function WeltNewsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = WeltNewsSDK.new(sdkopts)
  sdk.mode = "test"
