...
class WeltNewsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WeltNewsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WeltNewsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WeltNewsTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = WeltNewsTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WeltNewsTestRunner.getenv(key)
        ...
        explain = WeltNewsTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if WeltNewsTestRunner._test_control is not None:
            return WeltNewsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WeltNewsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WeltNewsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WeltNewsTestRunner.load_test_control()
...
def load_env_local():
    WeltNewsTestRunner.load_env_local()


def env_override(m):
    return WeltNewsTestRunner.env_override(m)


def entity_data(v):
    return WeltNewsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WeltNewsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WeltNewsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WeltNewsTestRunner.load_test_control()


def live_client_options():
    return WeltNewsTestRunner.live_client_options()


def live_delay_ms():
    return WeltNewsTestRunner.live_delay_ms()
