...
{
    private static function utility(): WeltNewsUtility
    {
        return new WeltNewsUtility();
    }
    ...
    private static function ctx(array $over = []): WeltNewsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new WeltNewsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new WeltNewsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof WeltNewsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new WeltNewsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = WeltNewsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new WeltNewsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = WeltNewsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new WeltNewsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = WeltNewsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WeltNewsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = WeltNewsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new WeltNewsResponse([]),
            'result' => new WeltNewsResult([]),
        ]);
        [, $err] = WeltNewsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new WeltNewsSpec([]),
            'response' => null,
            'result' => new WeltNewsResult([]),
        ]);
        [, $err] = WeltNewsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new WeltNewsSpec(['step' => 's']),
            'response' => new WeltNewsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new WeltNewsResult([]),
        ]);
        [, $err] = WeltNewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new WeltNewsSpec(['step' => 's']),
            'response' => new WeltNewsResponse(self::resp(200, ['v' => 1])),
            'result' => new WeltNewsResult([]),
        ]);
        [, $err] = WeltNewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new WeltNewsSpec(['step' => 's']),
            'response' => new WeltNewsResponse(self::resp(200, ['v' => 2])),
            'result' => new WeltNewsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        WeltNewsMakeResponse::call($ctx);
    ...
    {
        $preset = new WeltNewsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new WeltNewsSpec([]),
            'response' => new WeltNewsResponse([]),
            'result' => new WeltNewsResult([]),
        ...
        [$got, $err] = WeltNewsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new WeltNewsResult([])]);
        [, $err] = WeltNewsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new WeltNewsSpec([]), 'result' => null]);
        [, $err] = WeltNewsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WeltNewsSpec(['step' => 's']),
            'result' => new WeltNewsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new WeltNewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WeltNewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WeltNewsSpec(['step' => 's']),
            'result' => new WeltNewsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new WeltNewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WeltNewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WeltNewsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new WeltNewsSpec([]),
            'result' => new WeltNewsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = WeltNewsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WeltNewsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new WeltNewsError('boom', 'boom');
        $utility->fetcher = function (WeltNewsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WeltNewsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = WeltNewsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (WeltNewsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = WeltNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(WeltNewsResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (WeltNewsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WeltNewsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = WeltNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new WeltNewsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new WeltNewsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = WeltNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = WeltNewsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = WeltNewsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new WeltNewsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, WeltNewsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new WeltNewsResult(['ok' => false])]);
        $this->expectException(WeltNewsError::class);
        WeltNewsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new WeltNewsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', WeltNewsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        WeltNewsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new WeltNewsBaseFeature();
        $b = new WeltNewsBaseFeature();
        WeltNewsFeatureAdd::call($ctx, $a);
        WeltNewsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): WeltNewsBaseFeature
    {
        $f = new WeltNewsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        WeltNewsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): WeltNewsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new WeltNewsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WeltNewsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new WeltNewsResponse(['headers' => null]),
            'result' => new WeltNewsResult([]),
        ]);
        WeltNewsResultHeaders::call($ctx);
