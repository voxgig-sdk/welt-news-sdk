...
const TEST_SEED = { "entity": { "article": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+WeltNewsSDK\b/.test(code) ||
        /\bWeltNewsSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+WeltNewsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/WeltNewsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
