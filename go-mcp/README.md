...
[MCP](https://modelcontextprotocol.io) server exposing the WeltNews SDK as
two agent tools — `welt-news_list` and `welt-news_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/welt-news-mcp)
make build

# 2. Provide credentials via the environment
export WELT_NEWS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user welt-news \
  -- /absolute/path/to/welt-news-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./welt-news-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// welt-news_list: first page of records
{ "entity": "article" }
{ "entity": "article", "query": { } }

// welt-news_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/welt-news-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user welt-news \
     -- "$PWD"/dist/*/welt-news-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `welt-news_list` and `welt-news_load` tools now appear
   in new sessions. Ask the agent to *"list article using welt-news"*
   and it calls `welt-news_list` with `{"entity":"article"}`.
...
```sh
export WELT_NEWS_APIKEY=sk_live_xxx            # API key
export WELT_NEWS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `welt-news_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `welt-news_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WELT_NEWS_APIKEY` | API key sent with every request. |
| `WELT_NEWS_BASE` | Optional override of the API base URL. |
...
```sh
./welt-news-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"welt-news_load","arguments":{"entity":"article","query":{"id":1}}}}'
```
