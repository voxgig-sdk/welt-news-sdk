...
The Ruby SDK for the WeltNews API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
require_relative "WeltNews_sdk"
client = WeltNewsSDK.new(options)
```
...
raise a `WeltNewsError` (a `StandardError` subclass), so wrap
