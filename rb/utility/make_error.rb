...
module WeltNewsUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = WeltNewsContext.new({}, nil)
    end
    op = ctx.op || WeltNewsOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || WeltNewsResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(WeltNewsError) ? err.msg : err.to_s
    ...
    sdk_err = WeltNewsError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(WeltNewsError)
