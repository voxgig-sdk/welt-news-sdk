...
module WeltNewsUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = WeltNewsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WeltNewsResponse.new(fetched)
    else
