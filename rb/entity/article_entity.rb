...
class ArticleEntity
  ...
  def data_set(args)
    if args
      @_data = WeltNewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = WeltNewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Article>, Array] the matching Article items; raises WeltNewsError on failure
