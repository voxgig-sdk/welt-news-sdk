...
require_relative 'WeltNews_types'
...
class WeltNewsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WeltNewsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WeltNewsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WeltNewsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WeltNewsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WeltNewsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WeltNewsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => WeltNewsError.new(
        "#{op}_allow",
        "WeltNewsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = WeltNewsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WeltNewsError.new(
        "graphql_error", "WeltNewsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
